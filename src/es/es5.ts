...
export const es5 = {
    Program: (astPath: AstPath<ESTree.Program>): VistorFuncType => {
        const { node, scope, evaluate } = astPath; 
        node.body.forEach((bodyNode) => {
            evaluate({ node: bodyNode, scope, evaluate });
        });
    },
    ExpressionStatement: (astPath: AstPath<ESTree.ExpressionStatement>): VistorFuncType => {
        const { node, scope, evaluate } = astPath;
        evaluate({ node: node.expression, scope, evaluate });
    },
    BlockStatement: (astPath: AstPath<ESTree.BlockStatement>): VistorFuncType => {
        const { node, scope, evaluate } = astPath;
    ...
    ThisExpression: (astPath: AstPath<ESTree.ThisExpression>): VistorFuncType => {
        const { scope } = astPath;
        const thisVariable = scope.search('this');
        return thisVariable ? thisVariable.getVal() : null;
    },
    ObjectExpression: (astPath: AstPath<ESTree.ObjectExpression>): VistorFuncType => {
        const { node, scope, evaluate } = astPath;
        let result = {};
    ...
    ArrayExpression: (astPath: AstPath<ESTree.ArrayExpression>): VistorFuncType => {
        const { node, scope, evaluate } = astPath;
        const result = [];
    ...
    BinaryExpression: (astPath: AstPath<ESTree.BinaryExpression>): VistorFuncType => {
        const { node, scope, evaluate } = astPath;
    ...
    Literal: (astPath: AstPath<ESTree.Literal>): VistorFuncType => {
        const { node } = astPath;
    ...
    Identifier: (astPath: AstPath<ESTree.Identifier>): VistorFuncType => {
    ...
    VariableDeclaration: (astPath: AstPath<ESTree.VariableDeclaration>): VistorFuncType => {
        const { node, scope, evaluate } = astPath;
    ...
    VariableDeclarator: (astPath: AstPath<ESTree.VariableDeclarator>): VistorFuncType => {
        const { node, scope, evaluate } = astPath;
        const { id, init } = node;
        const key = (<ESTree.Identifier>id).name;
        const value = init ? evaluate({ node: init, scope, evaluate }) : undefined;
    ...
    MemberExpression: (astPath: AstPath<ESTree.MemberExpression>): VistorFuncType => {
        const { node, scope, evaluate } = astPath;
    ...
    CallExpression: (astPath: AstPath<ESTree.CallExpression>): VistorFuncType => {
        const { node, scope, evaluate } = astPath;
        const { callee } = node;
    ...
    WithStatement: (): VistorFuncType => {
    // with 就不实现了
        throw `${TAG} WithStatement is not implement`;
    },
    AssignmentExpression: (astPath: AstPath<ESTree.AssignmentExpression>): VistorFuncType => {
        const { node, scope, evaluate } = astPath;
    ...
    ForStatement: (astPath: AstPath<ESTree.ForStatement>): VistorFuncType => {
        const { node, scope, evaluate } = astPath;
    ...
    WhileStatement: (astPath: AstPath<ESTree.WhileStatement>): VistorFuncType => {
        const { node, scope, evaluate } = astPath;
    ...
    DoWhileStatement: (astPath: AstPath<ESTree.DoWhileStatement>): VistorFuncType => {
        const { node, scope, evaluate } = astPath;
        const { test, body } = node;
    ...
    ForInStatement: (astPath: AstPath<ESTree.ForInStatement>): VistorFuncType => {
        const { node, scope, evaluate } = astPath;
    ...
    UpdateExpression: (astPath: AstPath<ESTree.UpdateExpression>): VistorFuncType => {
        const { node, scope, evaluate } = astPath;
    ...
    BreakStatement: (): VistorFuncType => {
        return new Signal('break');
    },
    ContinueStatement: (): VistorFuncType => {
        return new Signal('continue');
    },
    ReturnStatement: (astPath: AstPath<ESTree.ReturnStatement>): VistorFuncType => {
        const { node, scope, evaluate } = astPath;
        return new Signal('return', node.argument ? evaluate({ node: node.argument, scope, evaluate }) : undefined);
    },
    IfStatement: (astPath: AstPath<ESTree.IfStatement>): VistorFuncType => {
        const { node, scope, evaluate } = astPath;
    ...
    FunctionExpression: (astPath: AstPath<ESTree.FunctionExpression>): VistorFuncType => {
        const { node, scope, evaluate } = astPath;
    ...
    FunctionDeclaration: (astPath: AstPath<ESTree.FunctionDeclaration>): VistorFuncType => {
        const { node, scope } = astPath;
        const { name } = node.id;
    ...
    ThrowStatement: (astPath: AstPath<ESTree.ThrowStatement>): VistorFuncType => {
        const { node, scope, evaluate } = astPath;
        throw evaluate({ node: node.argument, scope, evaluate });
    },
    TryStatement: (astPath: AstPath<ESTree.TryStatement>): VistorFuncType => {
    // TODO 缺少catch或finally的时候，抛错
        ...
        try {
            ...
            if (node.finalizer) evaluate({ node: node.finalizer, scope, evaluate });
        }
    },
    ConditionalExpression: (astPath: AstPath<ESTree.ConditionalExpression>): VistorFuncType => {
        const { node, scope, evaluate } = astPath;
        const { test, consequent, alternate } = node;
        return evaluate({ node: test, scope, evaluate }) ? 
            evaluate({ node: consequent, scope, evaluate }) : evaluate({ node: alternate, scope, evaluate });
    },
    LogicalExpression: (astPath: AstPath<ESTree.LogicalExpression>): VistorFuncType => {
        const { node, scope, evaluate } = astPath;
        const { left, operator, right } = node;
    ...
    UnaryExpression: (astPath: AstPath<ESTree.UnaryExpression>): VistorFuncType => {
        const { node, scope, evaluate } = astPath;
    ...
    DebuggerStatement: (): VistorFuncType => {
        // eslint-disable-next-line no-debugger
        debugger;
    },
    EmptyStatement: (): VistorFuncType => {
    // 啥都不做
    }
};